...
class ultraGravityFormBase {
    ...
    function ugf_page_template() {
        ...
        if(isset($_POST['ugfform_id'])) {
            ...
            $form_row = $wpdb->get_row("select * from " . $wpdb->prefix . "ugf_forms where id = '$form_id'");
            if(!$form_row) {
                return;
            }
            ...
            $fields_row = $wpdb->get_results("select * from " . $wpdb->prefix . "ugf_fields where form_id = '$form_id' order by ordering");
            for($i = 0; $i< count($fields_row);++$i) {
                ...
                $sql  = "select * from ". $wpdb->prefix. "ugf_fields_validations where field_id = '$field_id'";
                $validation_rows = $wpdb->get_results($sql);
    ...
    function ugf_manage_fields() {
        ...
        $sql = "select * from ". $wpdb->prefix . "ugf_forms where id = '$form_id'";
        $form_row = $wpdb->get_row($sql);
    ...
    function ugf_delete_field() {
        ...
        $form_id = $_POST['form_id'];
        $id = $_POST['id'];
        $sql = "delete from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$id'";;
        $wpdb->query($sql);
        
        $sql = "delete from " . $wpdb->prefix . "ugf_fields_validations where field_id = '$id'";;
        $wpdb->query($sql);
        
        
        $sql = "delete from " . $wpdb->prefix . "ugf_fields where id = '$id'";;
        $wpdb->query($sql);
    ...
    function ugf_fields_form() {
        ...
        $form_id = $_POST['form_id'];
        $id = $_POST['id'];
        $field_row =  $wpdb->get_row("select * from " . $wpdb->prefix . "ugf_fields where id ='$id'");
        if($field_row) {
    ...
    function ugf_fields_form_save_step2() {
        ...
        if($field_row->field_type == 'text') {
             $sql = "select id from " .$wpdb->prefix."ugf_fields_attributes where type='validation' and attribute_name = 'notempty' and field_id = '$id'";
        ...
        if($field_row->field_type == 'select' || $field_row->field_type == 'radio' || $field_row->field_type == 'checkbox') {
            $sql = "select * from " .$wpdb->prefix."ugf_fields_attributes where type='option' and attribute_name = 'options' and field_id = '$id'";
            $row = $wpdb->get_row($sql);
        ...
        if($field_row->field_type == 'image') {
            ...
            $sql = "select * from " .$wpdb->prefix."ugf_fields_attributes where type='option' and attribute_name = 'resize' and field_id = '$form_id'";
            $row = $wpdb->get_row($sql);
            ...
            $sql = "select * from " .$wpdb->prefix."ugf_fields_attributes where type='option' and attribute_name = 'width' and field_id = '$form_id'";
            $row = $wpdb->get_row($sql);
            ...
            $sql = "select *  from " .$wpdb->prefix."ugf_fields_attributes where type='option' and attribute_name = 'height' and field_id = '$form_id'";
             $row = $wpdb->get_row($sql);
            $html .= '<input type="hidden" name="id[]" value="' . $row->id .'">';
    ...
    function get_extra_form() {
        ...
        $checked = ' ';
        if($field_id > 0) {
            $sql = "select validation_type from " . $wpdb->prefix . "ugf_fields_validations where field_id = '$field_id' and validation_type = 'required'";
            $required_row = $wpdb->get_row($sql);
        ...
        if($type == 'text') {
            ...
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='title'";
            $attribute_value = $wpdb->get_var($sql);
        ...
        if($type == 'textarea') {
            ...
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='rows'";
            $attribute_value = $wpdb->get_var($sql);
            ...
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='cols'";
            $attribute_value = $wpdb->get_var($sql);
        ...
        if($type == 'select') {
            ...
            $html .= '</td>';
            $html .= '<td>';
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='options'";
            $attribute_value = $wpdb->get_var($sql);
            ...
            $html .= '</td>';
            $html .= '<td>';
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='default'";
            $attribute_value = $wpdb->get_var($sql);
            ...
            $html .= '</td>';
            $html .= '<td>';
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='first_row_blank'";
            $attribute_value = $wpdb->get_var($sql);
        ...
        if($type == 'date') {
            ...
            $html .= '</td>';
            $html .= '<td>';
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='date_format'";
            $attribute_value = $wpdb->get_var($sql);
            if(!$attribute_value) {
        ...
        if($type == 'image') {
            ...
            $html .= '</td>';
            $html .= '<td>';
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='resize'";
            $attribute_value = $wpdb->get_var($sql);
            $checked = '';
            if($attribute_value == 1) {
                $checked = 'checked';
            ...
            $html .= '</td>';
            $html .= '<td>';
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='width'";
            $attribute_value = $wpdb->get_var($sql);
            ...
            $sql = "select attribute_value from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id' and attribute_name='height'";
            $attribute_value = $wpdb->get_var($sql);
    ...
    function ugf_extra_attr_save() {
        ...
        if(is_array($type)) {
            for($j =0; $j < count($type); ++$j) {
                ...
                $data[] =   "type='" . $type[$j] ."'";
                $data[] =   "attribute_name='" . $attribute_name[$j] . "'";
                $data[] =   "attribute_value= '" . $attribute_value[$j] . "'";
                
                if($ids[$j] > 0) {
                    $sql = "update " .$wpdb->prefix . "ugf_fields_attributes set " . implode(",",$data) . " where id = '" . $ids[$j] . "'";
                    $wpdb->query($sql);                    
                }else {
            ...
            if($ids > 0) {
                $sql = "update " .$wpdb->prefix . "ugf_fields_attributes set " . implode(",",$data) . " where id = '$ids'";
                $wpdb->query($sql);
            }else {
                $sql = "insert into " .$wpdb->prefix . "ugf_fields_attributes set " . implode(",",$data);
                $wpdb->query($sql);
    ...
    function ugf_fields_attribute_list($field_id) {
        ...
        $field_attributes =  $wpdb->get_results("select * from " . $wpdb->prefix . "ugf_fields_attributes where field_id ='$field_id'");
    ...
    function ugf_fields_attribute_save() {
        ...
        $data[] = "id = '$attr_id'";
        $data[] = "field_id = '$field_id'";
        $data[] = "attribute_name = '$ugf_attribute_name'";
        $data[] = "attribute_value = '$ugf_attribute_value'";
        
        if($attr_id > 0) {
            $wpdb->query("update " . $wpdb->prefix . "ugf_fields_attributes set " . implode(",",$data) . " where id ='$attr_id'");
        }else {
            $wpdb->query("insert into  " . $wpdb->prefix . "ugf_fields_attributes set " . implode(",",$data));
        }
    ...
    function ugf_fields_attribute_del() {
        ...
        $field_id = $_POST['field_id'];
        $attr_id = $_POST['attr_id'];
        $wpdb->query("delete from  " . $wpdb->prefix . "ugf_fields_attributes where id ='$attr_id'");
        print $this->ugf_fields_attribute_list($field_id);
        die(0);
    }
    ...
    function ugf_fields_form_save() {
        ...
        $field_prefix_html = trim($_POST['field_prefix_html']);
        $field_postfix_html = trim($_POST['field_postfix_html']);
        $err = 0;
        ...
        if($err == 0) {
            ...
            $data[] = "field_machine_name = '$field_machine_name'";
            if(!$id) {
                $data[] = "field_type = '$field_type'";
            }
            $data[] = "lbl_placement = '$lbl_placement'";
            $data[] = "field_class = '$field_class'";
            
            
            $data[] = "lbl_wrapper_class = '$lbl_wrapper_class'";
            
            $data[] = "field_wrapper_class = '$field_wrapper_class'";
            
            $data[] = "field_prefix_html = '$field_prefix_html'";
            $data[] = "field_postfix_html = '$field_postfix_html'";
            $data[] = "label_prefix = '$label_prefix'";
            $data[] = "label_suffix = '$label_suffix'";
            
            if($id > 0) {
                $sql = "update " . $wpdb->prefix . "ugf_fields set " .implode(",",$data) . " where id = '$id'";
                $wpdb->query($sql);
                $output['id'] = $id;
            ...
            if(isset($attributes)) {
                foreach($attributes as $k => $v) {
                    if($k == 'title') {
                        $sql = "update " . $wpdb->prefix. "ugf_forms set post_title = '".$output['id']."' where id = '$form_id'";
                        $wpdb->query($sql);
                    }
                    $data = array();
                    $data[] = "field_id='" . $output['id'] . "'";
                    $data[] = "attribute_name='".$k."'";
    ...
    function get_form_fields_list($form_id) {
        global $wpdb;
        $url = plugins_url() . '/ultra-gravity-form';
        $sql = "select * from ". $wpdb->prefix . "ugf_fields where  form_id = '$form_id' order by ordering asc";
        $fields_rows = $wpdb->get_results($sql);
    ...
    function ugf_build_field_order() {
        ...
        for($i = 0; $i < count($order_arr); ++$i) {
            $id = $order_arr[$i];
            ++$o;
            $sql = "update " . $wpdb->prefix . "ugf_fields set ordering = '$o' where id = '$id' and form_id='$form_id'";
            $wpdb->query($sql);
        }
