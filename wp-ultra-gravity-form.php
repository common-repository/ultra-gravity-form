 ...
 */
/*
Plugin Name: Ultra Gravity Form
Description: Build Gravity form with ultra gravity form.
Author: ultrainfotek , ultrainfotek@gmail.com
Version: 1.5.4
*/
...
include_once(UGF_PATH . 'wp-ultra-gravity-form-base.php');



class ultraGravityForm extends ultraGravityFormBase { 
    ...
    function front_head() {
        ...
        $url = plugins_url();
        print '<link rel="stylesheet" type="text/css" href="'.$url.'/ultra-gravity-form/css/ultra-gravity-form.css" />';
        print '<script language="javascript" src="'.$url.'/ultra-gravity-form/js/ultra-gravity-form.js" /></script>';
    ...
    function ugf_menu() {
        ...
        add_menu_page('Ultra Gravity Form', 'Ultra Gravity Form', 'administrator', 'ugf', array($this,'ugf_main'));
    ...
    function ugf_form_load() {
        ...
        $html .= '<td><textarea name="form_prefix_html" rows="6"  cols="40">'.$form_prefix_html.'</textarea>';
        $html .= '</td>';    
        
        $html .= '</tr><tr>';
        $html .= '<td>';
        $html .= '<label class="ugf-label">Form Suffix HTML:</label></td>';
        $html .= '<td><textarea name="form_postfix_html" rows="6"  cols="40">' . $form_postfix_html . '</textarea>';
        $html .= '</td>';
        ...
        $html .= '<td><input type="text" name="captcha_msg" id="captcha_msg" size="40" value="'.$captcha_msg.'">';
        $html .= '</td>';
        ...
        $html .= '<td><textarea name="form_sucess_msg" id="form_sucess_msg" cols="40" rows="5">'.$form_sucess_msg.'</textarea>';
        $html .= '</td>';
    ...
    function form_delete_action() {
        global $wpdb;
        $form_id = $_POST['form_id'];
        $sql = "select id from  " .$wpdb->prefix . "ugf_fields where form_id = '$form_id'";
        $rows = $wpdb->get_results($sql);
        for($i = 0; $i < count($rows); ++$i) {
            $field_id = $rows[$i]->id;
            
            $sql = "delete from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id'";;
            $wpdb->query($sql);
            
            $sql = "delete from " . $wpdb->prefix . "ugf_fields_validations where field_id = '$field_id'";;
            $wpdb->query($sql);
            
            
        }
        
        $sql = "delete from " . $wpdb->prefix . "ugf_fields where form_id = '$form_id'";
        $wpdb->query($sql);
        
        $sql = "delete from " . $wpdb->prefix . "ugf_forms where id = '$form_id'";
        $wpdb->query($sql);
        print 1;
        die(0);
    }
    ...
    function ugf_form_save() {
        ...
        $post_method = trim($_POST['post_method']);
       
        $post_type = trim($_POST['post_type']);
       
        $form_classes = trim($_POST['form_classes']);
        $form_start_date = trim($_POST['form_start_date']);
        $form_end_date = trim($_POST['form_end_date']);
        $form_prefix_html = trim($_POST['form_prefix_html']);
        $form_postfix_html = trim($_POST['form_postfix_html']);
        $captcha = trim($_POST['captcha']);
        $captcha_msg = trim($_POST['captcha_msg']);
        $form_sucess_msg = trim($_POST['form_sucess_msg']);
        ...
        if($err == 0) {
            ...
            $data[] = "form_prefix_html = '$form_prefix_html'";
            $data[] = "form_postfix_html = '$form_postfix_html'";
            $data[] = "form_classes = '$form_classes'";
            $form_start_date = $this->dateToMysql($form_start_date);
            $data[] = "form_start_date = '$form_start_date'";
            $data[] = "captcha = '$captcha'";
            $data[] = "captcha_msg = '$captcha_msg'";
            $data[] = "form_sucess_msg = '$form_sucess_msg'";
            $data[] = "enable_form = '$enable_form'";
            
            
            
            $form_end_date = $this->dateToMysql($form_end_date);
            $data[] = "form_end_date = '$form_end_date'";
            
            if($form_id > 0) {
                $sql = "update " .$wpdb->prefix . 'ugf_forms set ' . implode(",",$data) . " where id = '$form_id'";
            }else {
...
function ugf_form($form_id) {
    ...
    */
    ...
    for($i =0; $i < count($fields_row); ++$i) {
        ...
        $sql = "select * from " . $wpdb->prefix . "ugf_fields_attributes where field_id = '$field_id'";
        $attr_row = $wpdb->get_results($sql);
        ...
        $validation_row = $wpdb->get_results($sql);
